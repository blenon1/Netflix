...
// show arrow event => Event Listener
function showArrow(leftArrow, rightArrow, carouselindex){
    const carousel = document.querySelector(carouselindex);
     carousel.addEventListener('mouseenter', () =>
     $(leftArrow).show(), // display: block;
     );
 
     carousel.addEventListener('mouseenter', () =>
     $(rightArrow).show(), // display: block;
     );
 };


// hide arrow event => Event Listener
function hideArrow(leftArrow, rightArrow, carouselindex){
    const carousel = document.querySelector(carouselindex);
     carousel.addEventListener('mouseleave', () =>
     $(leftArrow).hide(), // display: none;
     );
 
     carousel.addEventListener('mouseleave', () =>
     $(rightArrow).hide(), // display: none;
     );
 };
...
var comedyImg = new Array(
 ...
 var animeImg = new Array(
  ...
  // header navbar => background on y-scroll
  $(function() {
    
    $(window).on('scroll', function() {
        if($(window).scrollTop() > 80) {
            // add the background property
            $(".header-navbar").addClass("header-navbar-black");
        } else {
            // remove the background property
            $(".header-navbar").removeClass("header-navbar-black");
        }
    });
